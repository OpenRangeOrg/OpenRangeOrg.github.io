/*! googly.js 1.0 | (c)2024 Nihar Sarkar | sarkarnihar.in | 14th jan 2024
// https://github.com/NIHAR-SARKAR
*/ 
$(function (ex) {
    $('#txtremain').css('max-width', '300px');
    $('#result').text('250  Letters Remaining');
    $('textarea[data-validation]').keyup(function () {
        const isRemainValidation = $(this).attr('data-validation') == 'remain';
        if (isRemainValidation) {
            let attr = $(this).attr('maxlength');
            let id = $(this).attr('id');
            const max_len = (typeof attr !== 'undefined' && attr !== false) ? attr : 250;
            let c = $(this).val().length;
            let remaining = max_len - c;
            $('valid[for="' + id + '"]').text(remaining + ' Letters Remaining');
        }
    });
